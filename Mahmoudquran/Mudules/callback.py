...
from Mahmoudquran.Helpers import (
    _clear_,
    admin_check_cb,
    gen_thumb,
    is_streaming,
...
from Mahmoudquran.Helpers.dossier import *
from Mahmoudquran.Helpers.inline import (
    buttons,
    close_key,
    help_back,
    helpmenu,
    ...
    if CallbackQuery.from_user.id != int(user_id):
        try:
            return await CallbackQuery.answer(
                "⌔︙هـبـقـي افـضـل لـو بـقـيـت فـي حـدودك", show_alert=True
            )
        except:
            return
    ...
    if data == "resume_cb":
        if await is_streaming(query.message.chat.id):
            return await query.answer(
                "افتكرت انك وقفت البث؟", show_alert=True
            )
        ...
        if not await is_streaming(query.message.chat.id):
            return await query.answer(
                "افتكرت انك شغلت البث؟", show_alert=True
            )
