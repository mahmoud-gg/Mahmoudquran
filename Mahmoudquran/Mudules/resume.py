...
from Mahmoudquran import app, pytgcalls
from Mahmoudquran.Helpers import admin_check, close_key, is_streaming, stream_on
...
async def res_str(_, message: Message):
    ...
    if await is_streaming(message.chat.id):
        return await message.reply_text("** انت وقفتني ارسل كمل لاكمال السورة**")
